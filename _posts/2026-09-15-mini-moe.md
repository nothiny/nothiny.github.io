...
- 每层一个固定大小的 GPU 缓存（默认 LRU，策略可换，见下）。缺的专家在专用 CUDA stream 上 H2D，每个 slot 记 event，防止 staging 被复用、或 GPU 里的 expert 在 copy/kernel 还没结束时被淘汰。
...
（这一节是后来在 mini-deepseek 分支上补的，属于同一个子系统，顺手记在这里。）

对着真实 DeepSeek-V2-Lite 的 decode 流看计数器会发现：同前缀重复请求（radix 命中、纯 decode）的专家命中率只有 ~0.50。原因是路由流的形状：decode 每步激活 6 个专家、缓存只有 8 个，路由又在缓慢漂移，「最近使用序」和「即将使用序」错开，LRU 在循环访问下持续把马上要用的专家踢出去（经典 thrash）。

所以给 `ExpertResidentCache` 也做了一套可插拔淘汰策略（`--moe-expert-cache-eviction-policy`）：`lru`（默认，行为不变）、`lfu`（最低访问计数，recency 决胜）、`frequency-decay`（指数衰减频率，60s 半衰期）、`lru-k`（第 k 次访问最旧，不足 k 次的先淘汰）。一个关键设计是**替换元数据跨淘汰保留**：纯 LFU 在淘汰时清零计数，循环流下照样抖；被淘汰的专家重新载入时带着历史计数，热点集合才会稳定下来。

实测（V2-Lite，BF16 offload，每层缓存 8）：LFU 把同前缀请求的中位数从 3.41s 降到 2.88s（-15%），新前缀 7.80→6.01s（-23%），命中率 ~0.50→~0.55，每请求 H2D 字节 -10%。提升集中在 decode 相位；prefill 的 miss 是容量问题（一个 32 token 的 prompt 每层就能路由到 30+ 个不同专家），策略救不了。
...
`moe` 分支还顺带带了一套可插拔的 radix cache 淘汰策略（`lru` / `lfu` / `lru-k` / `frequency-decay` / `cost-aware` / `adaptive`），和 MoE 相对独立，这里不展开；后来在 mini-deepseek 分支上，expert 驻留缓存也照同样模式做了一套（见上一节）。
